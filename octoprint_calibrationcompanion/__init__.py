...
class calibrationcompanion(octoprint.plugin.SettingsPlugin,
    ...
    def get_received_gcode(self, comm, line, *args, **kwargs):
        if "Probe Offset X" in line:
            ...
            self.iteration += 1
            self._plugin_manager.send_plugin_message("calibrationcompanion", {"cycleIteration": self.iteration})
        elif "Kp:" in line and "Ki:" not in line and "Kd:" not in line and self._settings.get_boolean(["auto_apply"]):
            self._plugin_manager.send_plugin_message("calibrationcompanion", {
                "pidPConstant": re.findall("\d+\.\d+", line)})
        elif "Kp:" not in line and "Ki:" in line and "Kd:" not in line and self._settings.get_boolean(["auto_apply"]):
            self._plugin_manager.send_plugin_message("calibrationcompanion", {
                "pidIConstant": re.findall("\d+\.\d+", line)})
        elif "Kp:" not in line and "Ki:" not in line and "Kd:" in line and self._settings.get_boolean(["auto_apply"]):
            self._plugin_manager.send_plugin_message("calibrationcompanion", {
